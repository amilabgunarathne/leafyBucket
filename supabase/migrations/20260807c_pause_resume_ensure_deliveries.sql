-- Fix pause/resume: create current + next week deliveries if missing, then set open/paused.
-- Run AFTER 20260807_delivery_status_enum_paused.sql (paused enum must exist).
-- Safe to re-run.
...
BEGIN
  ...
  -- Ensure a delivery row exists for current Sunday and next Sunday
  FOR v_week IN
...
COMMENT ON FUNCTION public.set_my_subscription_paused(boolean, date) IS
  'Pause/resume subscription; create current+next week deliveries if missing; set them open or paused.';
