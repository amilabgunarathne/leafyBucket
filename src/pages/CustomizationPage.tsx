...
import { ArrowLeft, Check, X, Plus, Minus, Settings, AlertCircle, Calendar, Package, Wallet, RefreshCw, TrendingUp, Lock, Shield, TreePine, Leaf, Flower, Scale } from 'lucide-react';
import { Link } from 'react-router-dom';
import { vegetables, calculatePlanAllocation, defaultPlanVegetables, CATEGORY_VALUES, calculateTotalPlanWeight, getWeightBreakdownByCategory } from '../data/vegetables';
...
const CustomizationPage = () => {
  ...
  const getCurrentWeekLimits = () => {
    return WEEKLY_ADMIN_LIMITS[selectedPlan];
  };
  ...
  const getCurrentPlan = () => {
    const basePlan = basePlans[selectedPlan];
  ...
  const getDefaultVegetables = () => defaultPlanVegetables[selectedPlan];
  ...
  const toggleVegetableRemoval = (vegetableId) => {
  ...
  const toggleVegetableAddition = (vegetableId) => {
    if (!isCustomizationAllowed) return;
  ...
  const toggleVegetableExclusion = (vegetableId) => {
    if (!isCustomizationAllowed) return;
  ...
  const getCurrentAllocation = () => {
    ...
    return calculatePlanAllocation(currentPlan.vegetableBudget, currentVegetables);
  };

  // Calculate weight breakdown
  const getWeightBreakdown = () => {
    const currentVegetables = getCurrentVegetables();
    return getWeightBreakdownByCategory(currentVegetables);
  };
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="bg-white rounded-3xl p-8 shadow-lg">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  {Object.entries(basePlans).map(([planId, plan]) => {
                    const weekLimits = WEEKLY_ADMIN_LIMITS[planId];
                    const defaultVegs = defaultPlanVegetables[planId];
                    const totalWeight = calculateTotalPlanWeight(defaultVegs);
